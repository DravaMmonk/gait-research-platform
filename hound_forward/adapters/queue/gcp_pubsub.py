...
class PubSubJobQueue:
    ...
    def __init__(
        ...
        *,
        ...
        self._publisher, self._subscriber = self._build_clients(endpoint=endpoint)

    def enqueue(self, job: Job) -> None:
        topic_path = self._publisher.topic_path(self.project_id, self.topic)
        payload = json.dumps(serialize_job(job)).encode("utf-8")
        self._publisher.publish(topic_path, payload).result()

    def dequeue(self) -> Job | None:
        if not self.subscription:
            raise ValueError("PubSubJobQueue requires a subscription for dequeue operations.")
        subscription_path = self._subscriber.subscription_path(self.project_id, self.subscription)
        response = self._subscriber.pull(subscription=subscription_path, max_messages=1)
        if not response.received_messages:
            return None
        received = response.received_messages[0]
        payload = json.loads(received.message.data.decode("utf-8"))
        self._subscriber.acknowledge(subscription=subscription_path, ack_ids=[received.ack_id])
